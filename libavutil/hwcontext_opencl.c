                                 ...
                                 const AVFrame *src, int flags)
{
    ...
    if (device_priv->d3d11_map_amd) {
        cle = device_priv->clEnqueueAcquireD3D11ObjectsKHR(
            frames_priv->command_queue, 1, &desc->planes[desc->nb_planes - 1],
            0, NULL, &event);
        if (cle != CL_SUCCESS) {
            av_log(dst_fc, AV_LOG_ERROR, "Failed to acquire surface "
                   "handle: %d.\n", cle);
                                           ...
                                           AVHWFramesContext *src_fc, int flags)
{
    ...
    for (i = 0; i < frames_priv->nb_mapped_frames; i++) {
        ...
        if (device_priv->d3d11_map_amd) {
            ...
            // ref the unsplit AMD shared image at the end
            desc->planes[nb_planes - 1] = image = device_priv->clCreateFromD3D11Texture2DKHR(
            ...
            for (p = 0; p < nb_planes - 1; p++) {
                ...
                cl_mem planeUI = device_priv->clGetPlaneFromImageAMD(
                ...
                // convert from CL_UNSIGNED_INT8|16 to CL_UNORM_INT8|16
                if (image_fmt.image_channel_data_type == CL_UNSIGNED_INT8 ||
                    image_fmt.image_channel_data_type == CL_UNSIGNED_INT16) {
