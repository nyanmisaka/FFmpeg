...
int ff_vk_update_thread_context(AVCodecContext *dst, const AVCodecContext *src)
{
    ...
    err = av_buffer_replace(&dst_ctx->profile_ref, src_ctx->profile_ref);
    if (err < 0)
        return err;
                           ...
                           uint32_t *nb_slices, const uint32_t **offsets)
{
    FFVulkanDecodeContext *dec = avctx->internal->hwaccel_priv_data;
    FFVulkanDecodeShared *ctx = (FFVulkanDecodeShared *)dec->shared_ref->data;
    FFVulkanDecodeProfileData *prof = (FFVulkanDecodeProfileData *)dec->profile_ref->data;
...
int ff_vk_frame_params(AVCodecContext *avctx, AVBufferRef *hw_frames_ctx)
{
    ...
    dec->profile_ref = av_buffer_allocz(sizeof(FFVulkanDecodeProfileData));
    if (!dec->profile_ref) {
        av_buffer_unref(&dec->shared_ref);
        return AVERROR(ENOMEM);
    }

    prof = (FFVulkanDecodeProfileData *)dec->profile_ref->data;
    ...
    frames_ctx->internal->internal_ref = av_buffer_ref(dec->profile_ref);
    if (!frames_ctx->internal->internal_ref)
        return AVERROR(ENOMEM);
