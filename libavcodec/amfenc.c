...
static int amf_check_hevc_10bit_support(AVCodecContext *avctx)
{
    AMFEncContext *ctx = avctx->priv_data;
...
static int amf_init_encoder(AVCodecContext *avctx)
{
    ...
    if (pix_fmt == AV_PIX_FMT_P010) {
        switch (avctx->codec->id) {
            case AV_CODEC_ID_HEVC:
                // APU newer than Raven, GPU newer than Navi is required.
                res = amf_check_hevc_10bit_support(avctx);
                if (res == AMF_OK) {
                    ctx->bit_depth = 10;
                } else {
        ...
        case AV_PIX_FMT_D3D11:
            {
                ...
                //enum AMF_SURFACE_FORMAT surf_fmt_in;
